...
fdf$sd = fdf$median = fdf$mode = fdf$mean = NA
...
for (iimg in seq(nrow(fdf))){
	...
	# imgpos = imgpos[ord, ]
	####################
	# Must use pythag theorem 
	# for oblique acquisition 
	# not just diff
	### but should use diff if orthogonal 
	# - for example #
	# if negative
	...
	# print(posdiff )
	add.posdiff = c(posdiff, 
		posdiff[length(posdiff)])
	...
	has.varslice = 
		length(unique(thicks)) > 1

	ind = which(diff(thicks) != 0)
	thick.diff= which(
		abs(thicks - add.posdiff) > 1)
	ind = sort(unique(c(ind, thick.diff)))
	if (length(ind) > 0 ){
		ind = c(max(1, ind-1), ind, 
			min(ind+1, 
			length(thicks)))
		print(cbind(thicks, 
			add.posdiff)[ind,])
...
# }	
	...
	vals = df$value[df$Y == 1]

	nim = img.pred$nim
	dimg = dim(nim)
	
	# make sure z matches up
	nz = dimg[3]
	stopifnot( nz == nrow(dcmtables))
...
outfile = file.path(outdir, 
	"Reseg_111_Filenames_with_volumes.Rda"
	)
