...
fdf$img = gsub("^/dexter", 
	"/legacy/dexter", fdf$img)
fdf$ssimg = gsub("^/dexter", 
	"/legacy/dexter", fdf$ssimg)
fdf$iddir = gsub("^/dexter", 
	"/legacy/dexter", fdf$iddir)
...
for (iimg in seq(nrow(fdf))){
	...
	# man = unique(dcmtables[, 
	# 	"0008-0070-Manufacturer"])
	# stopifnot(length(man) == 1)
