...
##############################
fdf.run = fdf[ fdf$cnn_group == "training", ]
...
quants = vector(mode = "list", length = 2)
names(quants) = c("0", "1")

ind = which(all.df$Y == 1)
res = t(colQuantiles(as.matrix(all.df[ind, runnames]), 
             probs = c(0, 0.001, 0.005, 0.01, 
                       0.99, 0.995, 0.999, 1)))
quants$`1` = res

ind = which(all.df$Y == 0)
res = t(colQuantiles(as.matrix(all.df[ind, runnames]), 
                   probs = c(0, 0.001, 0.005, 0.01, 
                             0.99, 0.995, 0.999, 1)))
quants$`0` = res
...
fname = file.path(resdir, 
                  "iso_aggregate_data_cutoffs_cnn.rda")
...
fname = file.path(resdir, 
                  "iso_candidate_aggregate_data_cnn.rda")
